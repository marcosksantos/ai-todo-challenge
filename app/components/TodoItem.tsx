...
export default function TodoItem({ task, onUpdate }: { task: Task; onUpdate: () => void }) {
  ...
  return (
    <div className="group flex items-start gap-3 p-4 hover:bg-slate-800/30 transition">
      <button
      ...
      {isEditing ? (
        <div className="flex gap-2 flex-1 min-w-0">
          <input
            ...
            className="flex-1 bg-slate-800/50 px-3 py-1.5 rounded-lg border border-slate-700 text-white text-sm md:text-base focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 outline-none"
            autoFocus
          />
          <button
        ...
        <div className="flex items-center justify-between flex-1 min-w-0 gap-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <span className={`text-sm md:text-base leading-relaxed break-words flex-1 ${
            ...
            {task.is_ai_processing && (
              <span className="flex-shrink-0 ml-2 text-xs text-purple-400 bg-purple-400/10 px-2.5 py-1 rounded-full inline-flex items-center gap-1.5 border border-purple-400/20">
                ...
                <span className="hidden sm:inline">AI Optimizing...</span>
              </span>
            )}
          </div>
